...
pacman::p_load(
  ...
  zoo, splitstackshape, posterior, abind, wesanderson,
...
evaluate_nowcast <- function(dat, model, now, D_max = 35) {
  ...
  ts <- FHM_ICU %>%
    ...
    mutate(
      ...
      mean_7_c_lag = lag(mean_7_c, 7),   # ✅ Removed `fill = NA`
      lead_ind_cases_l = lag(mean_7_c, 18),
      lead_ind_cases_rl = lag(mean_7_c - mean_7_c_lag, 11),
      mean_7_i = rollmean(n_icu, 7, fill = NA, align = "center"),
      mean_7_i_lag = lag(mean_7_i, 7),
      lead_ind_icu_l = lag(mean_7_i, 14),
      lead_ind_icu_rl = lag((mean_7_i - mean_7_i_lag), 7)
    ) %>%
    mutate(across(where(is.numeric), ~replace_na(., 0))) %>%  # ✅ Replace NA values after mutation
    filter(
      date <= now,
      date >= start - 1
    )
  ...
  mod <- stan_model(file = paste0("./code/stan_models/", stan_mod, ".stan"))
  
  if (model %in% c("mod_r", "mod_r2")) {
    samples <- sampling(mod, data = list(
    ...
    ), seed = 1142, chains = 4, iter = 2000, control = list(adapt_delta = 0.98, max_treedepth = 15))
  ...
  # To extract diagnostics and summary:
  diagnostics <- summary(samples)$summary  # This is a matrix
  diagnostics_df <- as.data.frame(diagnostics)
  write_csv(diagnostics_df, paste0("./results/warnings/", model, "_", now, ".csv"))
  
  # Extract parameter summaries:
  summary_df <- summary(samples, pars = c("phi", "sigma", save_res))$summary
  summary_df <- as.data.frame(summary_df) %>% mutate(run_time = difftime(end_time, start_time, units = "mins"))
  write_csv(summary_df, paste0("./results/summary/", model, "_", now, ".csv"))
  
  # Extract draws and convert to long format:
  for (j in 1:length(save_res)) {
    draws_mat <- extract(samples, pars = save_res[j])[[1]]
    ...
    draws_wide <- draws_long %>%
      ...
      unnest(cols = everything()) %>%  # Expands list-cols into rows
      as.data.frame()
    
    write_csv(draws_wide, paste0("./results/", save_res[j], "/", save_res[j], "_", model, "_", now, ".csv"))
  }
}
